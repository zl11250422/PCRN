...
class ESA(nn.Module):
    def __init__(self, num_feat=50, conv=nn.Conv2d, p=0.25):
        ...
        #self.conv6 = nn.Conv2d(f, num_feat, 1)
    ...
    def forward(self, input):
        ...
        #y = self.contrast(input) + self.avg_pool(input)
...
class ESDB(nn.Module):
    ...
    def forward(self, input):
        ...
        r_c1, d_c1 = (self.pc1(input))
        r_c1 = self.act(r_c1 + input)

        #distilled_c2 = self.act(self.c2_d(r_c1))
        #r_c2 = (self.c2_r(r_c1))
        r_c2, d_c2 = (self.pc2(r_c1))
        r_c2 = self.act(r_c2 + r_c1)

        #distilled_c3 = self.act(self.c3_d(r_c2))
        #r_c3 = (self.c3_r(r_c2))
        r_c3, d_c3 = (self.pc3(r_c2))
...
class BSRN(nn.Module):
    ...
    def forward(self, input):
        input = torch.cat([input, input, input, input], dim=1)#[b, 12, h, w]
        out_fea = self.fea_conv(input)#[b, 64, h, w]
        ...
        out_B8 = self.B8(out_B7)#[b, 64, h, w]
        #par = sum(map(lambda x: x.numel(), self.convtest.parameters()))

        trunk = torch.cat([out_B1, out_B2, out_B3, out_B4, out_B5, out_B6, out_B7, out_B8], dim=1)#[b, 512, h, w]
        out_B = self.c1(trunk)#[b, 64, h, w]
        out_B = self.GELU(out_B)#[b, 64, h, w]
